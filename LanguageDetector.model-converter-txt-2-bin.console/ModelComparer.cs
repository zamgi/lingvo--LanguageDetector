...
namespace lingvo.ld.modelconverter
{
    ...
    /// 
    /// </summary>
    internal sealed class LanguageWeightPairEqualityComparer : IEqualityComparer< LanguageWeightPair >
    {
        public LanguageWeightPairEqualityComparer() { }
        public bool Equals( LanguageWeightPair x, LanguageWeightPair y ) => (x.Language == y.Language && x.Weight == y.Weight);
        public int GetHashCode( LanguageWeightPair obj ) => (obj.Language.GetHashCode() ^ obj.Weight.GetHashCode());
    }
    ...
    {
        private static LanguageWeightPair[] Intersect( LanguageWeightPair[] x, LanguageWeightPair[] y ) => x.Intersect( y, new LanguageWeightPairEqualityComparer() ).ToArray();
        ...
        unsafe public static void Compare( MModelBinaryNative model_1, MModelClassic model_2 )
        {
            ...
            using ( var e2 = dict2.GetEnumerator() )
            {
                ...
                for ( ; e1.MoveNext() && e2.MoveNext(); )
                {
                    ...
                    if ( !dict2.TryGetValue( s1.ToText(), out var bucketVal ) )
                    {
                        Debugger.Break();
                    }
    ...
    unsafe internal static class Extensions
    {
        ...
        public static int GetLength( this IntPtr _base ) => GetLength( (char*) _base );
        ...
        public static LanguageWeightPair[] ToLanguageWeightPairs( this IEnumerable< BucketRef > brs )
            => brs.Select( br => new LanguageWeightPair() { Language = br.Language, Weight = br.Weight } ).ToArray();

        public static WeighByLanguageNative ToWeighByLanguageNative( this IntPtr value ) => WeighByLanguageNative.Create( value );
    }
}
