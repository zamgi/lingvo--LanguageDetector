...
namespace lingvo.ld.MultiLanguage
{
    ...
    /// 
    /// </summary>
    unsafe internal interface INativeMemAllocationMediator : IDisposable
    {
        public IntPtr Alloc( int size );
        IntPtr AllocAndCopy( char* source, int sourceLength );
    }

    /// <summary>
    /// 
    /// </summary>
    unsafe internal sealed class NativeMemAllocationMediator : INativeMemAllocationMediator, IDisposable
    {
        /// <summary>
        /// 
        /// </summary>
    ...
    /// 
    /// </summary>
    unsafe internal sealed class NativeMemAllocationMediator_ThreadSafe : INativeMemAllocationMediator, IDisposable
    {
        private NativeMemAllocationMediator _NativeMemAllocator;
        public NativeMemAllocationMediator_ThreadSafe( int nativeBlockAllocSize ) => _NativeMemAllocator = new NativeMemAllocationMediator( nativeBlockAllocSize );
        public void Dispose() => _NativeMemAllocator.Dispose();

        [M(O.AggressiveInlining)] public IntPtr Alloc( int size )
        {
            lock ( _NativeMemAllocator )
            {
                return (_NativeMemAllocator.Alloc( size ));
            }
        }
        [M(O.AggressiveInlining)] public IntPtr AllocAndCopy( char* source, int sourceLength )
        {
            lock ( _NativeMemAllocator )
            {
                return (_NativeMemAllocator.AllocAndCopy( source, sourceLength ));
            }
        }
        public override string ToString() => _NativeMemAllocator.ToString();
    }
}
